##Sampling strategies (excluding those approaches, e.g. random and systematic, that are directly implemented in the simulations' script)
#Here are reported the data and functions used to perform the simulations of the sampling strategies
#Please, load packages in the "Bio_Elev_data"
...
#Function used (in Unif_sampl) to uniformly sampling points in the environmental space
#This function is also used in a parallel project
...
#of points are sampled in the environmental space than the number set by the user. The sample size is then adjusted
#internally in Unif_sampl to return the exact number of points
