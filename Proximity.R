...
Cor_dist_prob <- na.omit(data.frame(extract(D.sperandii.prob, coordinates(D.sperandii.prob.AOI), df = T),
                               extract(Abr_highway_AOI, coordinates(D.sperandii.prob.AOI), df = T)))

with(Cor_dist_prob, plot(layer, layer.1))
with(Cor_dist_prob, cor(layer, layer.1))
hist(with(Cor_dist_prob, layer*layer.1))
summary(with(Cor_dist_prob, layer*layer.1))

#do the same for the rare species
Cor_dist_prob.rare <- na.omit(data.frame(extract(D.tundrae.prob, coordinates(D.tundrae.bin.AOI), df = T),
                                    extract(Abr_highway_AOI, coordinates(D.tundrae.bin.AOI), df = T)))

with(Cor_dist_prob.rare, plot(layer, layer.1))
with(Cor_dist_prob.rare, cor(layer, layer.1)) #-0.1342271
hist(with(Cor_dist_prob.rare, layer*layer.1))
summary(with(Cor_dist_prob.rare, layer*layer.1))
